...
function print(table, rows, column, data) {
	for (var i = 0; i < rows; i++) {
		...
		for (var j = 0; j < column; j++) {
			...
			if ((j == 0 || j == column - 1 || i==rows-1) && i != 0) {
			hitball.table.rows[0].cells[0].innerHTML = "";
			hitball.table.rows[0].cells[0].style.backgroundColor = "green";
			}else if (i == 0) {
				hitball.table.rows[0].cells[0].innerHTML = "";
				hitball.table.rows[0].cells[0].style.backgroundColor = "red";
			}
			else
			{
				hitball.table.rows[0].cells[0].innerHTML = ".";
			...
			if(j == 0 || j == column - 1)
			{
			setCellDimension(0,0,"20px","40px");
			}
			else if(i ==0 || i ==rows-1)
			{	
			setCellDimension(0,0,"40px","20px");
			}
			else if((i==0 && j==0) || (i==0 && j==column-1) || (i==row-1 && j==0) || (i==row-1 && j==column-1) )
			{
			setCellDimension(0,0,"20px","20px");
			}
			else
			{
			setCellDimension(0,0,"40px","40px");
			}
		}
	}
...
function setCellDimension(row,col,width,height)
{

	hitball.table.rows[row].cells[col].style.width = width;
	hitball.table.rows[row].cells[col].style.height = height;
}
